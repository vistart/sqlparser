...
public class Lexer {
    ...
    public String info() {
        ...
        if (token == IDENTIFIER || token == LITERAL_ALIAS || token == LITERAL_CHARS) {
    ...
    public final void nextToken() {
        ...
        for (;;) {
            ...
            switch (ch) {
                ...
                case '#':
                    scanSharp();
                    if ((token == LINE_COMMENT || token == MULTI_LINE_COMMENT) && skipComment) {
                        bufPos = 0;
                        continue;
                    }
                    return;
        ...
        switch (ch) {
            ...
            case '!':
                ...
                if (ch == '=') {
                    ...
                    scanChar();
                    token = BANGBANG; // postsql
                } else if (ch == '~') {
                    scanChar();
                    if (ch == '*') {
                        scanChar();
                        token = BANG_TILDE_STAR; // postsql
                    } else {
                        token = BANG_TILDE; // postsql
                    }
                } else {
                    token = BANG;
                }
